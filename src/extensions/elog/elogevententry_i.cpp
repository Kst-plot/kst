...
ElogEventEntryI::ElogEventEntryI(KstELOG* elog,
                       QWidget* parent,
                       const char* name,
                       bool modal,
                       WFlags fl) : ElogEventEntry(parent, name, modal, fl) {
  _elog = elog;
}

ElogEventEntryI::~ElogEventEntryI() {
}

void ElogEventEntryI::initialize() {
  QGridLayout *grid;
...
void ElogEventEntryI::configuration() {
  _elog->doShow();
}

void ElogEventEntryI::customEvent(QCustomEvent* event) {
...
void ElogEventEntryI::submit() {
  saveSettings();
  _elog->submitEventEntry(i18n("Test of Event ELOG entry..."));
}

void ElogEventEntryI::setSettings() {
  //
...
void ElogEventEntryI::saveSettings() {
  KstELOGAttribStruct	attrib;
...
void ElogEventEntryI::loadSettings() {
  KConfig cfg("kstrc");
  QString str;
