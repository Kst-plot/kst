...
class ConfigWidgetLinearPlugin : public Kst::DataObjectConfigWidget, public Ui_LinearConfig {
  public:
    ...
    virtual void setupFromObject(Kst::Object* dataObject) {
      if (LinearSource* source = static_cast<LinearSource*>(dataObject)) {
...
//       }
  ...
  public slots:
    virtual void save() {
      if (_cfg) {
        _cfg->beginGroup("Interpolation Linear DataObject Plugin");
...
LinearSource::LinearSource(Kst::ObjectStore *store)
...
void LinearSource::change(Kst::DataObjectConfigWidget *configWidget) {
  if (ConfigWidgetLinearPlugin* config = static_cast<ConfigWidgetLinearPlugin*>(configWidget)) {
...
void LinearSource::setupOutputs() {
  setOutputVector(VECTOR_OUT, "");
}


bool LinearSource::algorithm() {
  ...
  return interpolate( inputVectorX, inputVectorY, inputVectorX1, outputVector, gsl_interp_linear);
}


Kst::VectorPtr LinearSource::vectorX() const {
  return _inputVectors[VECTOR_IN_X];
}


Kst::VectorPtr LinearSource::vectorY() const {
  return _inputVectors[VECTOR_IN_Y];
}


Kst::VectorPtr LinearSource::vectorX1() const {
  return _inputVectors[VECTOR_IN_X1];
}


QStringList LinearSource::inputVectorList() const {
...
QStringList LinearSource::inputScalarList() const {
  return QStringList( /*SCALAR_IN*/ );
}


QStringList LinearSource::inputStringList() const {
  return QStringList( /*STRING_IN*/ );
}


QStringList LinearSource::outputVectorList() const {
  return QStringList(VECTOR_OUT);
}


QStringList LinearSource::outputScalarList() const {
  return QStringList( /*SCALAR_OUT*/ );
}


QStringList LinearSource::outputStringList() const {
  return QStringList( /*STRING_OUT*/ );
}


void LinearSource::saveProperties(QXmlStreamWriter &s) {
  Q_UNUSED(s);
...
Kst::DataObjectConfigWidget *LinearPlugin::configWidget(QSettings *settingsObject) const {
  ConfigWidgetLinearPlugin *widget = new ConfigWidgetLinearPlugin(settingsObject);
  return widget;
}
