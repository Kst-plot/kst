...
dataVectorIndex=kst.DataVector(client,"/data/blast2003",field="INDEX",changeDetection=False,timeInterval=True,dontUpdate=False,start=157000,drange=1000,skip=0)
dataVectorGY1=kst.DataVector(client,"/data/blast2003",field="N15C5",changeDetection=False,timeInterval=True,dontUpdate=False,start=157000,drange=1000,skip=0)
...
GYHistCurve = kst.NewCurve(client,GYHist.X(), GYHist.Y(), curvecolor="black", curveweight=1, uselines=False, usebargraph=True,
                           bargraphfill="green",placeinexistingplot=P4)
